...
function getRelicFilterList(data, dispatch): Array<JSX.Element> {
    return data.map(element =>
        <div>
            <input
                type="checkbox"
...
function Filter({ filters, accessor }: FilterProps): JSX.Element {
    const dispatch = useFilterDispatch()
    const data = Object.entries(filters)
    dispatch({ type: "method", accessor: accessor })
